...
////////////////////////////////////////////////////////////////////////////
  ...
  *------------------------------------------------------------------------*/
  ...
  val cycleNum  = RegInit(0.U(log2Up(sigWidth + 3).W))
  ...
  val rem_Z     = Reg(UInt((sigWidth + 3).W))
  val sigX_Z    = Reg(UInt((sigWidth + 3).W))
  ...
  *------------------------------------------------------------------------*/
  ...
  val sigA_S = Cat(Fill(3, decA.sign), ~decA.sign, decA.fraction)
  // sigB_S = sigB << 2
  val sigB_S = Cat(decB.sign, ~decB.sign, decB.fraction, 0.U(2.W))
  ...
  *------------------------------------------------------------------------*/
  ...
  val scaleNotChange      = sigX_Z(sigWidth + 2) ^ sigX_Z(sigWidth)
  ...
  *------------------------------------------------------------------------*/
  ...
  val bitMask = Wire(UInt((sigWidth + 2).W))
  ...
  val trialTerm = Mux(ready && !io.sqrtOp, sigB_S,                                     0.U) |
                  Mux(ready &&  io.sqrtOp, (BigInt(1) << fracWidth).U,                 0.U) |
                  Mux(!ready && !sqrtOp_Z, Cat(signB_Z, ~signB_Z, fractB_Z, 0.U(2.W)), 0.U) | 
                  Mux(!ready &&  sqrtOp_Z, sigX_Z | 
                                           (bitMask & Fill(fracWidth, rem.head(1))) | 
                                           (bitMask >> 1),                             0.U)
  ...
  when(entering_normalCase || (!ready && newBit.asBool)) {
    sigX_Z := Mux(ready && !io.sqrtOp, newBit << (sigWidth + 2),  0.U) |
              Mux(ready &&  io.sqrtOp, (BigInt(1) << sigWidth).U, 0.U) |
  ...
  *------------------------------------------------------------------------*/
  ...
  val realFrac = Mux(
											scaleNotChange, 
											realSigX(fracWidth, 1), 
											realSigX(fracWidth - 1, 0)
										) 

  val decQ          = Wire(new RawFloat(nBits, eBits))
  val scaleNeedSub  = ~(realSigX(sigWidth + 2) ^ realSigX(sigWidth))
  val notNeedSubTwo = realSigX(sigWidth + 2) ^ realSigX(sigWidth - 1)
